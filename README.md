...
Minimal Agent 是一个可配置、可观测、支持多步骤规划与工具执行的 FastAPI Agent 原型，默认使用 mock 后端，支持 OpenAI、OpenAI-compatible 模型（含 Qwen）、Gemini、向量记忆、持久化工作流以及可选的 Plugin、Skill 和 MCP 运行时。

### OpenAI-compatible 模型（含 Qwen）
...
阿里云百炼/DashScope 的 Qwen 仍提供便捷预设：设置 `AGENT_LLM_BACKEND=qwen`，配置 `DASHSCOPE_API_KEY`；`QWEN_MODEL` 默认为 `qwen-plus`，`DASHSCOPE_BASE_URL` 默认为 `https://dashscope.aliyuncs.com/compatible-mode/v1`，可按地域或业务空间覆盖。
